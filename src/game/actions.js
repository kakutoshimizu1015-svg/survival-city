...
import { checkNpcCollision } from './npc'; // ▼ 追加
...
export const actionRollDice = async (isCpuCall = false) => {
    ...
    if (cp.equip?.bicycle) totalAP += 2; // 自転車ボーナス
...
export const executeMove = (targetTileId) => {
    ...
    if (tile.type === "event") {
        if (cp.isCPU) {
            if (Math.random() < 0.5) state.updateCurrentPlayer(p => ({ p: p.p + 5 }));
        } else {
            if (Math.random() < 0.3) {
                ...
                useGameStore.setState({ mgActive: true, mgType, mgValue: Math.floor(Math.random() * 14), mgResult: null });
            }
        }
    }
...
export const actionJob = () => {
    const state = useGameStore.getState();
    const cp = state.players[state.turn];
    const chance = cp.charType === "sales" ? 0.8 : 0.6;
...
export const actionOccupy = () => {
    const state = useGameStore.getState();
    const cp = state.players[state.turn];
    const pCost = (state.territories[cp.pos] !== undefined) ? 5 : 3; // 敵陣地は5P
...
export const actionEndTurn = async () => {
    ...
    state.updateCurrentPlayer(p => ({ ap: 0, stealth: false })); // ターン終了でステルス解除
    useGameStore.setState({ turn: nextTurn, diceRolled: false, isBranchPicking: false });
