...
export const getScenarioSteps = (id) => {
    ...
    switch(id) {
        case 0: return [
            ...
            { title:'🏆 スコア計算', icon:'🧮', text:'あなたのスコアを計算してみましょう：<br><br>• <b>所持P × 2</b> = 10 × 2 = <em>20点</em><br>• <b>陣地の価値</b> = 商業2マス = <em>8点</em><br>• <b>資源価値</b> = 缶3個 + ゴミ1個 = <em>5点</em><br>• <b>キル×3</b> = 2 × 3 = <em>+6点</em><br>• <b>デス×5</b> = 1 × 5 = <em>−5点</em><br>• <b>合計: 34点</b>', highlight: '#left-status-area' },
        ...
        case 1: return [
            ...
            { title:'🎮 ターンの流れ', icon:'🚶', text:'移動できました！マップ上のコマが動いたのが分かりますか？<br>APが残っている限り、<b>何度でも行動</b>できます。<br>もう一度移動するか、他のアクションを試してみてください。', nextLabel: '理解した ▶' },
        ...
        case 2: return [
            ...
            { title:'📊 画面ガイド', icon:'🗺️', text:'中央の<b>マップエリア</b>です。<br>左上のボタンでズーム操作、ドラッグで視点移動できます。<br>自由に操作してみてください！', highlight: '#board-area' }
        ];
        case 3: return [
            { title:'🗺️ マス体験', icon:'🥫', text:'あなたは<b>空き缶マス</b>にいます。<br>「🥫 缶拾い」ボタンを押して缶を拾ってみましょう！<br>（1AP消費、1ターン3回まで）', setup: () => initSandboxGame({ p1pos:2, p1ap:10, p1p:3, diceRolled:true }), highlight: '#btn-can', waitMode: '👆 缶拾いしてください', watchCondition: () => s().players[0].cans > 0 },
            { title:'🗺️ マス体験', icon:'✅', text:'缶を拾いました！缶は<b>買取所マス</b>で換金できます。<br>次は移動して<b>バイトマス</b>(💼)を目指しましょう。<br>「移動」ボタンで進んでください。', nextLabel: '次へ ▶' },
            { title:'🗺️ マス体験', icon:'💼', text:'<b>バイトマス</b>では3APを使って働けます。<br>成功すると<b>12P</b>獲得！（成功率は60〜80%）<br>バイトマスまで移動して「💼 バイト」ボタンを試してみてください。', waitMode: '👆 バイトマスで「バイト」を実行', watchCondition: () => { const tile = s().mapData.find(t => t.id === s().players[0].pos); return tile && tile.type === 'job' && s().players[0].p > 3; }},
        ];
        case 4: return [
            ...
            { title:'🃏 カード体験', icon:'🔴', text:'最後は<b>「レンガ」</b>（武器カード・射程2）です。<br>使うと<b>照準UI</b>が出て、角度を調整して攻撃できます。<br>CPUが射程2以内にいるので試してみてください。', waitMode: '👆 レンガカードを使用', watchCondition: () => !s().players[0].hand.includes(17) }
        ];
        case 5: return [
            { title:'👥 NPC体験', icon:'🗺️', text:'マップ上にNPCが配置されています。<br>移動してNPCがいるマスに入ると<b>自動でイベント</b>が発生します。<br>まずはマップを確認してみましょう。', setup: () => initSandboxGame({ p1pos:0, p1ap:10, p1p:8, p1hp:80, p1hand:[0], diceRolled:true, policePos:5, friendPos:4, p2pos:9 }), highlight: '#board-area' },
            { title:'👥 NPC体験', icon:'🤝', text:'マス4に<b>🤝 仲間NPC</b>がいます。<br>仲間のマスに止まると<b>良い効果</b>（HP回復やPボーナス）が得られます。<br>移動してマス4まで行ってみてください。', waitMode: '👆 仲間NPCのマスへ移動', watchCondition: () => s().players[0].pos === 4 },
            { title:'👥 NPC体験', icon:'🚓', text:'マス5に<b>🚓 警察</b>がいます。<br>警察のマスに止まると<b>補導</b>されて次ターンAP-2ペナルティ。<br>「ステルス行動」カードを先に使えば回避できます！<br>試しに近づいてみましょう。', nextLabel: '理解した ▶' }
        ];
        case 6: return [
