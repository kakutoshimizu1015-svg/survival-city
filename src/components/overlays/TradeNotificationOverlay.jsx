                ...
                <div key={trade.id} style={{ background: 'rgba(26, 13, 0, 0.95)', border: '2px solid #f1c40f', borderRadius: '8px', padding: '15px', color: '#fdf5e6', boxShadow: '0 4px 12px rgba(0,0,0,0.6)' }}>
                    ...
                    <div style={{ display: 'flex', gap: '10px', fontSize: '12px', marginBottom: '15px' }}>
                        <div style={{ flex: 1, background: 'rgba(52, 152, 219, 0.2)', padding: '8px', borderRadius: '6px', border: '1px solid #3498db' }}>
                            ...
                            <div>{trade.offer.p > 0 && <span>P: {trade.offer.p}<br/></span>}</div>
                            <div>{trade.offer.cans > 0 && <span>缶: {trade.offer.cans}<br/></span>}</div>
                            {trade.offer.skins.map(s => <div key={s}>👕 {s.replace('_',' ')}</div>)}
                            {trade.offer.p === 0 && trade.offer.cans === 0 && trade.offer.skins.length === 0 && 'なし'}
                        </div>
                        ...
                        <div style={{ flex: 1, background: 'rgba(231, 76, 60, 0.2)', padding: '8px', borderRadius: '6px', border: '1px solid #e74c3c' }}>
                            ...
                            <div>{trade.request.p > 0 && <span>P: {trade.request.p}<br/></span>}</div>
                            <div>{trade.request.cans > 0 && <span>缶: {trade.request.cans}<br/></span>}</div>
                            {trade.request.skins.map(s => <div key={s}>👕 {s.replace('_',' ')}</div>)}
                            {trade.request.p === 0 && trade.request.cans === 0 && trade.request.skins.length === 0 && 'なし'}
                        </div>
                    </div>
