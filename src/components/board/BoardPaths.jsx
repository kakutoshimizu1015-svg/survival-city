...
export const BoardPaths = () => {
    ...
    const TILE = 80, GAP = 20, PAD = 30, STEP = TILE + GAP, R = TILE / 2;
    ...
    mapData.forEach(tile => {
        const fx = tcx(tile), fy = tcy(tile);
        tile.next.forEach(nextId => {
            const next = mapData.find(t => t.id === nextId);
            if (!next) return;
            const tx = tcx(next), ty = tcy(next);
            ...
            arrows.push({ id: `${tile.id}-${nextId}`, x1, y1, x2, y2 });
        });
    });
    ...
    return (
        <svg
            ...
            height={H}
            style={{
                pointerEvents: 'none', display: 'block', overflow: 'visible',
                background: 'none', position: 'absolute', top: 0, left: 0, zIndex: 2
            }}
        >
            <defs>
                <marker id="arr-bk" viewBox="0 0 14 14" refX="7" refY="7" markerWidth="5" markerHeight="5" orient="auto">
