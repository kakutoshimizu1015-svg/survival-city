...
export const BoardPaths = React.memo(() => {
    const mapData = useGameStore(state => state.mapData);

    // useMemo で重いSVGパスの計算結果をキャッシュする
    const svgContent = useMemo(() => {
        ...
        // 【最適化】検索(find)を高速化するためのインデックスマップを作成
        const mapIndex = new Map(mapData.map(t => [t.id, t]));
            ...
            >
                ...
                {mapData.map(tile => {
                    ...
                    return tile.next.map(nextId => {
                        // 【最適化】findの代わりにMapからO(1)で取得
                        const nextTile = mapIndex.get(nextId);
