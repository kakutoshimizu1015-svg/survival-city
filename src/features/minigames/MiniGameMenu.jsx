...
export default function MiniGameMenu() {
    // ▼ 追加: 稼いだポイントをリアルタイムで確認するためのステートを取得
    const { gachaPoints, gachaCans } = useUserStore();
    ...
    return (
        <div style={{
            ...
            display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px 20px 40px', zIndex: 1000
        }}>
            {/* ▼ 上部ステータスバー */}
            <div style={{
                display: 'flex', justifyContent: 'center', gap: '15px', width: '100%', maxWidth: '500px',
                background: '#241a0e', padding: '10px', borderRadius: '10px', border: '1px solid #5a4228',
                marginBottom: '10px', boxShadow: '0 4px 10px rgba(0,0,0,0.5)'
            }}>
                <div style={{ color: '#2ecc71', fontWeight: 'bold', fontSize: '1.1rem' }}>🥫 {gachaCans}</div>
                <div style={{ color: '#e8b84b', fontWeight: 'bold', fontSize: '1.1rem' }}>💰 {gachaPoints} P</div>
            </div>

            <p style={{ marginTop: '0.5rem', fontSize: '0.7rem', letterSpacing: '0.3em', color: '#7a6a4a' }}>脱・ホームレスサバイバルシティ</p>
            <h1 style={{ fontFamily: '"Bebas Neue", sans-serif', fontSize: 'clamp(2.5rem, 9vw, 4.5rem)', lineHeight: 0.9, color: '#c97b2a', textShadow: '0 0 40px rgba(201,123,42,0.4), 4px 4px 0 #000', textAlign: 'center', margin: '0.5rem 0' }}>路上<br />ミニゲーム</h1>
            ...
            <div style={{
                ...
                width: '100%', maxWidth: '500px', marginTop: '10px'
            }}>
                {gamesList.map(g => (
                    <div key={g.id} onClick={() => goGame(g.id)} style={{
                        ...
                        boxShadow: '0 4px 6px rgba(0,0,0,0.3)', transition: 'transform 0.1s, border-color 0.1s'
                    }} 
                    onMouseOver={(e) => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.borderColor = '#c97b2a'; }} 
                    onMouseOut={(e) => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.borderColor = '#3d2e1a'; }}
                    >
            ...
            <div style={{ marginTop: '30px', display: 'flex', gap: '15px' }}>
                ...
                <button 
                    onClick={() => useGameStore.setState({ gamePhase: 'gacha' })}
                    style={{
                        padding: '12px 24px', borderRadius: '8px',
                        background: 'linear-gradient(135deg, #d35400, #e67e22)', border: 'none', color: '#fff',
                        cursor: 'pointer', fontWeight: 'bold', fontSize: '1rem', boxShadow: '0 4px 10px rgba(230,126,34,0.4)'
                    }}
                >🔥 ガチャを引く</button>
            </div>
        </div>
