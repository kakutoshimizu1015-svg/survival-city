...
export function RatGame({ pts, addPts, onBack, isEventMode }) {
    ...
    const { time, start, stop } = useTimer(10, () => { if (!isSpectator && playingRef.current) endRat(); }, isSpectator);
    ...
    const spawnRat = useCallback(() => {
        ...
        if (edge === 0) { x = rnd(20, W - 20); y = 10; } else if (edge === 1) { x = W - 10; y = rnd(20, H - 20); } else if (edge === 2) { x = rnd(20, W - 20); y = H - 10; } else { x = 10; y = rnd(20, H - 20); }
        ratsRef.current.push({ id: idRef.current++, x, y, alive: true, speed: rnd(10, 18) / 10 });
    }, [isSpectator]);
    ...
    const animate = useCallback(() => {
        ...
        ratsRef.current.forEach(r => {
            if (!r.alive) return;
            const cx = (arenaRef.current?.offsetWidth || 300) / 2, cy = (arenaRef.current?.offsetHeight || 280) / 2;
            const dx = cx - r.x, dy = cy - r.y, dist = Math.hypot(dx, dy);
            if (dist < 25) {
                r.alive = false; stolenRef.current += 2; stolenOccurred = true; addGachaAssets(0, -2);
            ...
            } else { r.x += (dx / dist) * r.speed; r.y += (dy / dist) * r.speed; r.flip = dx > 0; }
        });
    ...
    const hitRat = (e, id) => {
        ...
        if (isSpectator || !playingRef.current) return;
        const r = ratsRef.current.find(x => x.id === id); if (!r || !r.alive) return;
        r.alive = false; hitRef.current++; setHitCount(hitRef.current);
        const fid = Date.now() + Math.random(); setFxList(prev => [...prev, { id: fid, x: r.x, y: r.y, t: '💥', c: '#e8b84b' }]);
    ...
    const endRat = useCallback(() => {
        ...
        const win = stolenRef.current <= 3; const p = win ? Math.max(0, hitRef.current * 2 - stolenRef.current) : 0;
        setResult({ win, icon: win ? '🐀' : '💀', main: win ? 'ネズミ撃退成功！' : '盗まれすぎた…', sub: `撃退${hitRef.current}匹 / 損失${stolenRef.current}P`, pts: p });
        if (p > 0) addPts(p);
    ...
    const startGame = () => {
        if (isSpectator) return;
        setIsReady(true); playingRef.current = true; 
        for (let i = 0; i < 4; i++) spawnRat();
        start(); rafRef.current = requestAnimationFrame(animate);
    };
    ...
    // ▼ 同期
    useEffect(() => {
        if (!isSpectator) useGameStore.setState({ mgSyncState: { isReady, hitCount, stolen, rats, result } });
    }, [isReady, hitCount, stolen, JSON.stringify(rats), JSON.stringify(result), isSpectator]);
            ...
            <div style={S.body}>
                {!isReady && !result ? (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
                        <Instr><strong style={{color:'#e8b84b', fontSize:'1.1rem'}}>【ルール説明】</strong><br/><br/>10秒間、中央の荷物💰に迫ってくるネズミを直接タップして撃退しろ！<br/>盗まれた被害が3P以下なら成功だ！</Instr>
                    ...
                    <>
                        ...
                        <div ref={arenaRef} className="rat-arena">
                            ...
                            {rats.map(r => <div key={r.id} className="rat-el" onPointerDown={(e) => hitRat(e, r.id)} style={{ left: r.x, top: r.y, transform: r.flip ? 'scaleX(-1)' : 'scaleX(1)' }}>🐀</div>)}
...
export function DrunkGame({ pts, addPts, onBack, isEventMode }) {
    ...
    const { time, start, stop } = useTimer(10, () => { if (!isSpectator && playingRef.current) endDrunk(true); }, isSpectator);

    const endDrunk = useCallback((survived) => {
        if (isSpectator) return;
        playingRef.current = false; stop(); cancelAnimationFrame(rafRef.current); clearInterval(keepIvRef.current);
        const win = survived && keepRef.current >= 6; const p = win ? Math.floor(keepRef.current * 1.5) : 0;
        setResult({ win, icon: win ? '🎉' : '🌀', main: win ? 'バランスキープ成功！' : '倒れた！', sub: `緑ゾーン内 ${keepRef.current}秒 / 6秒以上で成功`, pts: p });
        if (p > 0) addPts(p);
    }, [isSpectator, stop, addPts]);

    const animate = useCallback(() => {
        if (isSpectator || !playingRef.current) return;
        driftRef.current += (Math.random() - 0.5) * 0.06; driftRef.current = Math.max(-0.25, Math.min(0.25, driftRef.current)); 
        balRef.current += driftRef.current; setBal(balRef.current);
        if (balRef.current <= 0 || balRef.current >= 100) { endDrunk(false); return; }
        rafRef.current = requestAnimationFrame(animate);
    }, [isSpectator, endDrunk]);

    const tap = (e, d) => {
        e.preventDefault(); if (isSpectator || !playingRef.current) return;
        balRef.current = Math.max(0, Math.min(100, balRef.current - d * 10)); driftRef.current -= d * 0.15; setBal(balRef.current);
    };
    ...
    const startGame = () => {
        if (isSpectator) return;
        setIsReady(true); playingRef.current = true; start();
        rafRef.current = requestAnimationFrame(animate);
        keepIvRef.current = setInterval(() => {
            if (playingRef.current && balRef.current >= 35 && balRef.current <= 65) { keepRef.current++; setKeep(keepRef.current); }
        }, 1000);
    };
    ...
    // ▼ 同期
    useEffect(() => {
        if (!isSpectator) useGameStore.setState({ mgSyncState: { isReady, bal, keep, result } });
    }, [isReady, bal, keep, JSON.stringify(result), isSpectator]);
    ...
    return (
        <div style={{ ...S.screen, pointerEvents: isSpectator ? 'none' : 'auto' }}>
            ...
            <div style={S.body}>
                {!isReady && !result ? (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
                        <Instr><strong style={{color:'#e8b84b', fontSize:'1.1rem'}}>【ルール説明】</strong><br/><br/>酔っ払いがフラフラと倒れそうになるぞ！<br/>左に傾いたら「右タップ」、右に傾いたら「左タップ」してバランスを取れ。<br/>緑のゾーン内に合計6秒以上いればクリアだ！</Instr>
                        <BtnPrim onClick={startGame} style={{ width: '100%' }}>ゲーム開始！</BtnPrim>
                    </div>
                ) : (
                    <>
                        ...
                        <div style={{ width: '100%' }}>
                            ...
                            <div className="balance-bar-track"><div className="balance-center" /><div className="balance-zone" /><div className="balance-indicator" style={{ left: `${bal}%` }} /></div>
                        </div>
                        {!result && (
...
export function RainGame({ pts, addPts, onBack, isEventMode }) {
    ...
    const { time, start, stop } = useTimer(10, () => { if (!isSpectator && playingRef.current) endRain(wetRef.current < 100); }, isSpectator);

    const endRain = useCallback((win) => {
        if (isSpectator) return;
        playingRef.current = false; stop(); clearInterval(wetIvRef.current); cancelAnimationFrame(rafRef.current);
    ...
    const spawnObs = useCallback(() => {
        if (isSpectator || !playingRef.current) return;
        const target = OBS_LIST[rnd(0, OBS_LIST.length - 1)]; setInfo(`⚠️ ${target.name}が来る！`); 
        let ox = 120; let hit = false;
        const tick = () => {
            if (!playingRef.current) return;
            ox -= 2.5; setObs({ x: ox, e: target.e });
            if (!hit && ox < 25 && ox > 12) {
                hit = true;
                if (!jumpRef.current) {
                    wetRef.current = Math.min(100, wetRef.current + target.danger); setWet(wetRef.current); setInfo(`💥 ${target.name}にぶつかった！`);
                    if (wetRef.current >= 100) { endRain(false); return; }
                } else { setInfo('🌟 ジャンプ成功！'); }
            }
            if (ox < -20) { setObs({ x: -100, e: null }); setTimeout(() => { if (playingRef.current) spawnObs(); }, rnd(600, 1000)); return; }
    ...
    const doJump = useCallback((e) => {
        e.preventDefault(); if (isSpectator || !playingRef.current || jumpRef.current) return;
        jumpRef.current = true; setJumping(true); setTimeout(() => { if (playingRef.current) { jumpRef.current = false; setJumping(false); } }, 500); 
    }, [isSpectator]);

    const init = useCallback(() => {
        if (isSpectator) return;
        playingRef.current = false; cancelAnimationFrame(rafRef.current); clearInterval(wetIvRef.current);
        jumpRef.current = false; wetRef.current = 0; setWet(0); setJumping(false); setObs({ x: -100, e: null }); setInfo('障害物を待て…'); setResult(null); setIsReady(false);
    }, [isSpectator]);
    ...
    const startGame = () => {
        ...
        wetIvRef.current = setInterval(() => {
            if (playingRef.current) { wetRef.current = Math.min(100, wetRef.current + 1.5); setWet(wetRef.current); if (wetRef.current >= 100) endRain(false); }
        }, 400);
    ...
    // ▼ 同期
    useEffect(() => {
        if (!isSpectator) useGameStore.setState({ mgSyncState: { isReady, wet, obs, info, jumping, result } });
    }, [isReady, wet, JSON.stringify(obs), info, jumping, JSON.stringify(result), isSpectator]);
    ...
    return (
        <div style={{ ...S.screen, pointerEvents: isSpectator ? 'none' : 'auto' }}>
            ...
            <div style={S.body}>
                {!isReady && !result ? (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
                        <Instr><strong style={{color:'#e8b84b', fontSize:'1.1rem'}}>【ルール説明】</strong><br/><br/>雨宿り先までダッシュしろ！<br/>途中で障害物が向かってくるので、「JUMP！」ボタンで避けろ。<br/>濡れ度が100%になる前に10秒間逃げ切ればクリアだ！</Instr>
                        <BtnPrim onClick={startGame} style={{ width: '100%' }}>ゲーム開始！</BtnPrim>
                    </div>
                ) : (
                    <>
                        ...
                        <div className="rain-arena">
                            {playingRef.current && Array.from({ length: 8 }).map((_, i) => <div key={i} className="rain-drop" style={{ left: `${rnd(0, 100)}%`, animationDuration: `${rnd(6, 12) / 10}s`, animationDelay: `${rnd(0, 10) / 10}s` }}>|</div>)}
...
export function KashiGame({ pts, addPts, onBack, isEventMode }) {
    ...
    const { time, start, stop } = useTimer(10, () => { if (!isSpectator && playingRef.current) endKashi(); }, isSpectator);

    const endKashi = useCallback(() => {
        if (isSpectator) return;
        playingRef.current = false; stop(); clearInterval(bentoIvRef.current); cancelAnimationFrame(rafRef.current);
        const win = scoreRef.current >= 3; const p = win ? scoreRef.current * 3 : 0;
    ...
    const animate = useCallback(() => {
        if (isSpectator || !playingRef.current) return;
        if (moveDirRef.current !== 0) { pxRef.current = Math.max(5, Math.min(95, pxRef.current + moveDirRef.current * 1.5)); setPlayerX(pxRef.current); }
        for (let i = bentosRef.current.length - 1; i >= 0; i--) {
            const b = bentosRef.current[i]; b.y += 1.8; 
            if (b.y > 90) { bentosRef.current.splice(i, 1); continue; }
            if (b.y > 75 && !b.hit) {
                if (Math.abs(b.x - pxRef.current) < 8) { scoreRef.current++; setScore(scoreRef.current); b.hit = true; bentosRef.current.splice(i, 1); } 
                else if (Math.abs(b.x - rxRef.current) < 8) { rivalRef.current++; setRival(rivalRef.current); b.hit = true; bentosRef.current.splice(i, 1); }
            }
        }
        if (bentosRef.current.length > 0) {
            const target = [...bentosRef.current].sort((a, b) => b.y - a.y)[0];
            if (target) { rxRef.current += (target.x > rxRef.current ? 1.2 : -1.2); rxRef.current = Math.max(5, Math.min(95, rxRef.current)); setRivalX(rxRef.current); }
        }
        setBentos([...bentosRef.current]); rafRef.current = requestAnimationFrame(animate);
    }, [isSpectator]);

    const handlePointerDown = (e, dir) => { e.preventDefault(); if(!isSpectator) moveDirRef.current = dir; };
    const handlePointerUp = (e) => { e.preventDefault(); if(!isSpectator) moveDirRef.current = 0; };

    const init = useCallback(() => {
        if (isSpectator) return;
        playingRef.current = false; cancelAnimationFrame(rafRef.current); clearInterval(bentoIvRef.current);
        moveDirRef.current = 0; pxRef.current = 40; rxRef.current = 68; scoreRef.current = 0; rivalRef.current = 0; bentosRef.current = []; idRef.current = 0;
    ...
    const startGame = () => {
        if (isSpectator) return;
        setIsReady(true); playingRef.current = true; start();
        bentoIvRef.current = setInterval(spawnBento, 1000); rafRef.current = requestAnimationFrame(animate);
    };
    ...
    // ▼ 同期
    useEffect(() => {
        if (!isSpectator) useGameStore.setState({ mgSyncState: { isReady, score, rival, playerX, rivalX, bentos, result } });
    }, [isReady, score, rival, playerX, rivalX, JSON.stringify(bentos), JSON.stringify(result), isSpectator]);
    ...
    return (
        <div style={{ ...S.screen, pointerEvents: isSpectator ? 'none' : 'auto' }}>
            ...
            <div style={S.body}>
                {!isReady && !result ? (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
                        <Instr><strong style={{color:'#e8b84b', fontSize:'1.1rem'}}>【ルール説明】</strong><br/><br/>上から落ちてくる弁当をライバルより先にキャッチしろ！<br/>◀▶ボタンを押し続けて移動だ。<br/>10秒以内に3個キャッチできればクリア！</Instr>
                        <BtnPrim onClick={startGame} style={{ width: '100%' }}>ゲーム開始！</BtnPrim>
                    </div>
                ) : (
                    <>
