...
export const TILE_COLORS = {
    "normal":     { top: "#6b7f52", side: "#4d5e3a", hi: "#8a9e6a" },
    "can":        { top: "#8a9b3e", side: "#6a7b2e", hi: "#a0b050" },
    "trash":      { top: "#9a7a2e", side: "#7a5a1e", hi: "#b89840" },
    "shop":       { top: "#4a8a9a", side: "#3a6a7a", hi: "#5aa0b0" },
    "exchange":   { top: "#7a4a9a", side: "#5a3a7a", hi: "#9a60b0" },
    "job":        { top: "#9a5a3e", side: "#7a4a2e", hi: "#b07050" },
    "koban":      { top: "#9a4a5a", side: "#7a3a4a", hi: "#b06070" },
    "event":      { top: "#9a3e9a", side: "#7a2e7a", hi: "#b050b0" },
    "shelter":    { top: "#3e8a4a", side: "#2e6a3a", hi: "#50b060" },
    "manhole":    { top: "#3a3a3a", side: "#2a2a2a", hi: "#505050" },
    "center":     { top: "#aa3a3a", side: "#8a2a2a", hi: "#c05050" },
};

// 疑似3D座標・Z軸（奥行き）を付与するヘルパー関数
function apply3DCoordinates(tiles) {
    if (tiles.length === 0) return tiles;
    const maxRow = Math.max(...tiles.map(t => t.row));
    return tiles.map(tile => {
        // 奥（rowが小さい）ほどZ値を大きくする (0.0〜7.2の範囲を想定)
        const z = Math.max(0, (maxRow - tile.row) * 1.15);
        // Zによるスケールを考慮しつつX, Y座標を決定 (TileW=60, Gap=20と想定)
        const x = tile.col * 80 + 30; // colに基づく絶対X座標
        const y = tile.row * 80 + 30; // rowに基づく絶対Y座標
        return { ...tile, x, y, z };
    });
}

export function genSmallMap(){
    const map = [
